...
namespace Perfect_Launcher
{
    public partial class Form1 : Form
    {
        ...
        // Espelha o teclado/mouse da conta ativa para todas as outras contas abertas.
        ...
        public Form1()
        {
            ...
            // O cliente 64-bit pode estar tanto na própria pasta quanto na subpasta "x64"
            // (layout de servidores como o "Classic PW", que só têm cliente 64-bit).
            bool bClientFound = File.Exists(Application.StartupPath + "\\" + Exe32)
                || File.Exists(Application.StartupPath + "\\" + Exe64)
                || File.Exists(Application.StartupPath + "\\x64\\" + Exe64);
            ...
            // Se NÃO existe cliente 32-bit, esta instalação só roda em 64-bit,
            // então força o modo 64-bit (senão o launcher tentaria abrir um
            // ELEMENTCLIENT.EXE que não existe).
        ...
        public void OpenGame(int UserId, bool bOnlyAdd = false, int ProcessId = -1, bool IgnorarAbertas = false)
        {
            ...
            // Garante que a pasta "userdata" e o arquivo accounts.txt existam antes de
            // mexer nos atributos/escrever (senão dá DirectoryNotFound em instalações novas).
            var pastaUserdata = Application.StartupPath + "\\userdata";
            if (!Directory.Exists(pastaUserdata))
                Directory.CreateDirectory(pastaUserdata);
            if (!File.Exists(caminhoArquivoAccounts))
        ...
        private async Task<bool> HasUpdate()
        {
            try
            {
                ...
                // Este verificador de updates depende do layout do PW-BR (pastas irmãs
                // "patcher" e "config" ao lado da pasta "element"). Servidores com outro
                // layout não têm esses arquivos; nesse caso não há como verificar, então
                // assumimos que está atualizado e não incomodamos o usuário com erros.
                string updateServerPath = Application.StartupPath.Replace("\\element", "\\patcher\\server\\updateserver.txt");
                string versionSwPath = Application.StartupPath.Replace("\\element", "\\config\\element\\version.sw");
                if (!File.Exists(updateServerPath) || !File.Exists(versionSwPath))
                    return false;
                ...
                string TextoComUrl = File.ReadAllText(updateServerPath);
                ...
                CVersion = File.ReadAllText(versionSwPath);
